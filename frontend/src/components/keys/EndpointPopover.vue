...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    ...
    >
      ...
      <ElPopover  :trigger="['hover', 'focus']"  :width="256" placement="top" :show-after="100" :hide-after="150"><template #reference><div class="group/endpoint relative flex items-center gap-1.5" tabindex="0"><code
        ...
        >{{ item.endpoint }}</code><ElButton text
          native-type="button"
        ...
        </a></div></template><div class="w-max max-w-[24rem] rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-left shadow-[0_14px_36px_-20px_rgba(15,23,42,0.35)] ring-1 ring-slate-200/80 transition-all duration-150 dark:border-slate-700 dark:bg-slate-900 dark:ring-slate-700/70"><p
