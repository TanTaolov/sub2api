<template>
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <ElCard shadow="never" class="element-surface-card p-4 flex items-center gap-3">
    ...
    <ElCard shadow="never" class="element-surface-card p-4 flex items-center gap-3">
      ...
      <div>
        ...
        <p class="flex flex-wrap items-center gap-x-1 text-xs text-gray-500">
          ...
          <span>/</span>
          <ElPopover  :trigger="['hover', 'focus']"  :width="224" placement="top" :show-after="100" :hide-after="150"><template #reference><span class="group relative inline-flex cursor-help items-center gap-0.5" tabindex="0" ><span>{{ cacheLabel() }}: {{ formatTokens(stats?.total_cache_tokens || 0) }}</span><svg
            ...
            >
              ...
              />
            </svg></span></template><div class="w-56 rounded-lg border border-gray-200 bg-white p-3 text-left text-xs text-gray-700 shadow-lg dark:border-dark-600 dark:bg-dark-800 dark:text-dark-200"><span class="mb-2 block font-medium text-gray-900 dark:text-white">
                {{ cacheDetailLabel() }}
              </span><span class="flex items-center justify-between gap-3">
                <span>{{ t('usage.cacheCreationTokensLabel') }}</span>
                <span class="tabular-nums">
                  {{ formatTokens(stats?.total_cache_creation_tokens || 0) }}
                </span>
              </span><span class="mt-1 flex items-center justify-between gap-3">
