<template>
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <ElForm v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      ...
      <div>
        ...
        <div class="relative flex gap-2">
          <div class="relative flex-1"><div class="absolute left-3 top-1/2 -translate-y-1/2 font-medium text-gray-500">$</div><ElementInput v-model.number="form.amount" type="number" step="any" min="0" required class="input pl-8" /></div>
          <ElButton v-if="operation === 'subtract'" native-type="button" @click="fillAllBalance" class="whitespace-nowrap">{{ t('admin.users.withdrawAll') }}</ElButton>
        </div>
      </div>
      <div><label class="input-label">{{ t('admin.users.notes') }}</label><ElementInput type="textarea" v-model="form.notes" :rows="3" class="input"></ElementInput></div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <ElButton @click="$emit('close')" class="">{{ t('common.cancel') }}</ElButton>
        <ElButton native-type="submit" form="balance-form" :disabled="submitting || !form.amount" class="" :class="operation === 'add' ? 'bg-emerald-600 text-white' : 'btn-danger'">{{ submitting ? t('common.saving') : t('common.confirm') }}</ElButton>
      </div>
    </template>
  </BaseDialog>
</template>
