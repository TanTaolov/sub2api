<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <ElForm v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      <div>
        <label class="input-label">{{ t('admin.users.email') }}</label>
        <ElementInput v-model="form.email" type="email" class="input" />
      </div>
      <div>
        <label class="input-label">{{ t('admin.users.password') }}</label>
        <div class="flex gap-2">
          <div class="relative flex-1">
            <ElementInput v-model="form.password" type="text" class="input pr-10" :placeholder="t('admin.users.enterNewPassword')" />
            <ElButton text v-if="form.password" native-type="button" @click="copyPassword" class="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1 transition-colors hover:bg-gray-100 dark:hover:bg-dark-700" :class="passwordCopied ? 'text-green-500' : 'text-gray-400'">
          ...
          <ElButton native-type="button" @click="generatePassword" class="px-3">
      ...
      <div>
        ...
        <ElementInput v-model="form.username" type="text" class="input" />
      </div>
      <div>
      ...
      <div>
        ...
        <ElementInput type="textarea" v-model="form.notes" :rows="3" class="input"></ElementInput>
      </div>
      <div>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <ElButton type="primary" native-type="submit" form="edit-user-form" :disabled="submitting" class="">
