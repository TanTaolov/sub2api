<template>
  <div class="flex items-center gap-2">
    ...
    <ElPopover v-if="hasError && account.error_message" :trigger="['hover', 'focus']"  :width="256" placement="top" :show-after="100" :hide-after="150"><template #reference><div class="group/error relative" tabindex="0"><svg
      ...
      >
        ...
        />
      </svg><!-- Tooltip - 向下显示 --></div></template><div class="min-w-[200px] max-w-[300px] rounded-lg bg-gray-800 px-3 py-2 text-xs text-white shadow-xl transition-all duration-200 dark:bg-gray-900"><div class="whitespace-pre-wrap break-words leading-relaxed text-gray-300">
    ...
    <ElPopover v-if="isRateLimited" :trigger="['hover', 'focus']"  :width="224" placement="top" :show-after="100" :hide-after="150"><template #reference><div class="group relative" tabindex="0"><span
        class="inline-flex items-center gap-1 rounded bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
      >
        <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
        429
      </span><!-- Tooltip --></div></template><div class="w-56 whitespace-normal rounded bg-gray-900 px-3 py-2 text-center text-xs leading-relaxed text-white transition-opacity dark:bg-gray-700">{{ t('admin.accounts.status.rateLimitedUntil', { time: formatDateTime(account.rate_limit_reset_at) }) }}
        <div
    ...
    >
      <ElPopover v-for="item in activeModelStatuses" :key="`${item.kind}-${item.model}`" :trigger="['hover', 'focus']"  :width="256" placement="top" :show-after="100" :hide-after="150"><template #reference><div class="group relative mb-1 break-inside-avoid" tabindex="0"><!-- 积分已用尽 --><span
        ...
        >
          ...
          {{ formatScopeName(item.model) }}
          <span class="text-[10px] opacity-70">{{ formatCountdown(item.reset_at) }}</span>
        </span><!-- Tooltip --></div></template><div class="w-max max-w-[320px] whitespace-nowrap rounded bg-gray-900 px-3 py-2 text-center text-xs leading-relaxed text-white transition-opacity dark:bg-gray-700">{{
    ...
    <ElPopover v-if="isOverloaded" :trigger="['hover', 'focus']"  :width="224" placement="top" :show-after="100" :hide-after="150"><template #reference><div class="group relative" tabindex="0"><span
        class="inline-flex items-center gap-1 rounded bg-red-100 px-1.5 py-0.5 text-xs font-medium text-red-700 dark:bg-red-900/30 dark:text-red-400"
      >
        <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
        529
      </span><!-- Tooltip --></div></template><div class="w-56 whitespace-normal rounded bg-gray-900 px-3 py-2 text-center text-xs leading-relaxed text-white transition-opacity dark:bg-gray-700">{{ t('admin.accounts.status.overloadedUntil', { time: formatTime(account.overload_until) }) }}
        <div
          class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-gray-900 dark:border-t-gray-700"
        ></div></div></ElPopover>
  </div>
