<template>
  <div class="space-y-4">
    ...
    <ElCard shadow="never" v-else-if="initError" class="element-surface-card p-6 text-center">
      <p class="text-sm text-red-600 dark:text-red-400">{{ initError }}</p>
      <ElButton class="mt-4" @click="$emit('back')">{{ t('payment.result.backToRecharge') }}</ElButton>
    </ElCard>
    <!-- Success -->
    <template v-else-if="success">
      <ElCard shadow="never" class="element-surface-card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <ElButton type="primary" class="" @click="$emit('done')">{{ t('common.confirm') }}</ElButton>
        </div>
      </ElCard>
    </template>
    <template v-else>
      ...
      <ElCard shadow="never" class="element-surface-card p-6">
        ...
        <ElButton class="btn-stripe mt-6 w-full py-3 text-base" :disabled="submitting || !ready" @click="handlePay">
