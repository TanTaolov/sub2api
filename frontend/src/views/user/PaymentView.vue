<template>
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6">
      ...
      <template v-else>
        ...
        <div v-if="tabs.length > 1 && paymentPhase === 'select' && !selectedPlan" class="flex space-x-1 rounded-xl bg-gray-100 p-1 dark:bg-dark-800">
          <ElButton text v-for="tab in tabs" :key="tab.key"
            class="flex-1 rounded-lg px-4 py-2.5 text-sm font-medium transition-all"
            :class="activeTab === tab.key ? 'bg-white text-gray-900 shadow dark:bg-dark-700 dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'"
            @click="activeTab = tab.key">{{ tab.label }}</ElButton>
        </div>
        ...
        <template v-else>
          ...
          <ElCard shadow="never" v-if="tabs.length === 0" class="element-surface-card py-16 text-center">
          ...
          <template v-else-if="activeTab === 'recharge'">
            ...
            <ElCard shadow="never" v-if="enabledMethods.length === 0" class="element-surface-card py-16 text-center">
            ...
            <ElCard shadow="never" v-if="enabledMethods.length >= 1" class="element-surface-card p-6">
            ...
            <ElCard shadow="never" v-if="validAmount > 0" class="element-surface-card p-6">
            ...
            <ElButton text :class="['btn w-full py-3 text-base font-medium', paymentButtonClass]" :disabled="!canSubmit || submitting" @click="handleSubmitRecharge">
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <template v-if="selectedPlan">
              ...
              <ElCard shadow="never" v-if="feeRate > 0 && selectedPlan.price > 0" class="element-surface-card p-6">
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
              ...
              <ElButton text :class="['btn w-full py-3 text-base font-medium', paymentButtonClass]" :disabled="!canSubmitSubscription || submitting" @click="confirmSubscribe">
                <span v-if="submitting" class="flex items-center justify-center gap-2">
                  <span class="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></span>
                  {{ t('common.processing') }}
                </span>
              ...
              <ElButton class="w-full" @click="selectedPlan = null">{{ t('common.cancel') }}</ElButton>
            </template>
            <!-- Plan list -->
            <template v-else>
              <ElCard shadow="never" v-if="checkout.plans.length === 0" class="element-surface-card py-16 text-center">
        ...
        <ElCard shadow="never" v-if="(checkout.help_text || checkout.help_image_url) && paymentPhase === 'select' && !selectedPlan" class="element-surface-card p-4">
    ...
    <ElDialog :model-value="Boolean(showRenewalModal)" :title="t('payment.selectPlan')" width="512px" append-to-body align-center destroy-on-close :show-close="true" :close-on-click-modal="true" :close-on-press-escape="true" class="element-dialog " @update:model-value="visible => { if (!visible) { closeRenewalModal() } }">
<template v-if="showRenewalModal"><!-- Close button --><ElButton text class="absolute right-4 top-4 rounded-lg p-1 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-dark-700 dark:hover:text-gray-200" @click="closeRenewalModal">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </ElButton><div class="min-h-0 space-y-4 overflow-y-auto">
              <SubscriptionPlanCard v-for="plan in renewalPlans" :key="plan.id" :plan="plan" :active-subscriptions="activeSubscriptions" @select="selectPlanFromModal" />
            </div></template>
</ElDialog>
    <!-- Image Preview Overlay -->
    <ElImageViewer v-if="previewImage" :url-list="[previewImage]" teleported hide-on-click-modal @close="previewImage = ''" />
  </AppLayout>
</template>
