  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <ElCard shadow="never" class="element-surface-card">
        <ElTabs :model-value="activeTab" @update:model-value="value => switchTab(value as DetailTab)" class="element-page-tabs px-4"><ElTabPane v-for="tab in detailTabs" :key="tab.key" :name="tab.key"><template #label><Icon :name="tab.icon" size="sm" />
            {{ tab.label }}</template></ElTabPane></ElTabs>
        ...
        <UsageFilters v-model="filters" ref="usageFiltersRef" flat :mode="activeTab" class="border-b border-gray-100 dark:border-dark-700/50" :start-date="startDate" :end-date="endDate" :exporting="exporting" :model-options="modelNameOptions" @change="applyFilters" @refresh="refreshData" @reset="resetFilters" @cleanup="openCleanupDialog" @export="exportToExcel">
          <template #after-reset>
            <ElementFloatingPanel v-if="activeTab !== 'ranking'" :visible="Boolean(showColumnDropdown)"  width="192" @close="showColumnDropdown = false"><template #reference><div class="relative" ref="columnDropdownRef"><ElButton
