...
func TestHandleGeminiStreamingResponse_EmptyAndDoneEventBoundaries(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, tc := range []struct {
		...
		{name: "empty_before_data", body: "data:\n\ndata: {\"candidates\":[]}\n\n"},
	} {
		for _, ending := range []struct {
			...
			t.Run(tc.name+"/"+ending.name, func(t *testing.T) {
				...
				resp := &http.Response{
					StatusCode: http.StatusOK,
					Body:       io.NopCloser(strings.NewReader(strings.ReplaceAll(tc.body, "\n", ending.value))),
					Header:     http.Header{},
				}
				...
				require.NoError(t, err)
				require.NotNil(t, result)
				require.Equal(t, tc.body, rec.Body.String(), "empty and terminal data events must retain their dispatch boundary")
			})
		}
	}
}
